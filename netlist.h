...
在对网标第一次遍历后，需要一个数据结构存储所有被驱动节点具体到比特的信息
需要维护节点比特被驱动所在的行数
...
class NetAssign : public NetAssignBase {
    ...
    public:
      ...
      bool is_asynchronous();    //asynchronous  异步
 ...
 * Block Proc的表
 * NOTE: The emit method calls the target->proc_block function but               //recurse 递归
...
class NetBlock  : public NetProc {
    ...
    public:
      ...
      void append(NetProc*);            //后加过程
      void prepend(NetProc*);           //前加过程
	...
	// synthesize as asynchronous logic, and return true.                                //异步逻辑综合
      bool synth_async(Design*des, NetScope*scope,                            
		       NexusSet&nex_map, NetBus&nex_out,
		       NetBus&enables, vector<mask_t>&bitmasks);

      bool synth_sync(Design*des, NetScope*scope,                                         //同步逻辑综合
		      bool&ff_negedge,
 ...
 */
class NetCondit  : public NetProc {                                           //条件语句
...
class Design {
	...
	// Iterate over the design...
      ...
      void recurseTraversal(NetProc*  , int );
      void examples(void);
